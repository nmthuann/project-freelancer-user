...
import { UpdateProfileUserDto } from './profile-user-dto/update-profileUser.dto';
...
export class ProfileUserService {
  constructor(
    ...
    async createProfileUser(profileDto: ProfileUserDto) {
        ...
        .where("infor.infor_id = :infor_id", { infor_id: profileDto.infor.infor_id })
        .getOne();

        const findInfor = await this.informationUserService.getInformationUserById(profileDto.infor.infor_id);
        findInfor.profile= newProfile;
        await this.informationUserService.updateInformationUserById(profileDto.infor.infor_id, findInfor);
        return newProfile;  
    }

   async getInforIdByProfileId(email: string): Promise<number>{

        const findInfor = await this.informationUserService.getInforIdByEmail(email);
